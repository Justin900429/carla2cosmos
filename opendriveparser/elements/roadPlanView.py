...
class PlanView(object):
    ...
    def addParamPoly3(self, startPosition, heading, length, aU, bU, cU, dU, aV, bV, cV, dV, pRange):
        self._geometries.append(
            ParamPoly3(startPosition, heading, length, aU, bU, cU, dU, aV, bV, cV, dV, pRange)
        )
...
class Spiral(Geometry):
    def __init__(self, startPosition, heading, length, curvStart, curvEnd):
        ...
        self._spiral = eulerspiral.EulerSpiral.createFromLengthAndCurvature(
            self._length, self._curvStart, self._curvEnd
        )
    ...
    def calcPosition(self, s):
        (x, y, t) = self._spiral.calc(
            s, self._startPosition[0], self._startPosition[1], self._curvStart, self._heading
        )
