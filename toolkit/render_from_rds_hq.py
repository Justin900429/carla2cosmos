...
def prepare_input(
    ...
    for camera_name in settings["CAMERAS"]:
        ...
        if MAX_CHUNK > 0:
            ...
            print(
                f"Processing {clip_id} {camera_name} with {len(render_frame_ids)} frames only due to max_chunk={MAX_CHUNK}"
            )
...
def prepare_output(
    ...
    if full_video.shape[1] != resize_resolution[1] or full_video.shape[2] != resize_resolution[0]:
        print(
            f"Resizing {clip_id} {render_name} video from {full_video.shape[1]}x{full_video.shape[2]} to {resize_resolution[1]}x{resize_resolution[0]}..."
        )
        full_video = np.stack(
            [cv2.resize(frame, resize_resolution, interpolation=cv2.INTER_LANCZOS4) for frame in full_video],
            axis=0,
        )
    ...
    for cur_idx, i in enumerate(range(0, len(render_frame_ids), TARGET_CHUNK_FRAME - OVERLAP_FRAME)):
        ...
        output_writer = imageio_v1.get_writer(
            ...
            ffmpeg_params=[
                ...
                "-preset",
                "slow",  # Slower preset for better compression/quality
                "-pix_fmt",
                "yuv420p",  # Ensures wide compatibility
            ],
        )
    ...
    for camera_name, camera_model in camera_name_to_camera_model.items():
        ...
        for frame_idx in render_frame_ids:
            ...
            for i in range(len(accumulate_idx)):
                lidar_points = (
                    lidar_data[f"{low_fps_frame_indices[i]:06d}.lidar_raw.npz"]["xyz"]
                    .astype(np.float32)
                    .reshape(-1, 3)
                )
                ...
                lidar_points_world = (
                    lidar_to_world
                    @ np.concatenate([lidar_points, np.ones([lidar_points.shape[0], 1])], axis=1).T
                ).T[:, :3]
                accumulated_points.append(torch.tensor(lidar_points_world, device="cuda"))
                original_bbox_dicts.append(
                    all_object_info[f"{low_fps_frame_indices[i]:06d}.all_object_info.json"]
                )
...
def main(input_root, clip_id_json, output_root, dataset, camera_type, skip, post_training, novel_pose_folder):
    ...
    if post_training:  # for post-training only
        assert dataset in ["waymo", "waymo_mv", "waymo_mv_short"], (
            "post_training is only supported for waymo dataset"
        )
