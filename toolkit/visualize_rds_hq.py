...
def world_coordinate_to_bev_image_coordinate(
    points_in_world, world_to_ego, bev_map_h_pixel, bev_map_w_pixel, bev_map_h_meter, bev_map_w_meter
):
    """
    ...
    points_in_ego = (
        world_to_ego @ np.concatenate([points_in_world, np.ones((len(points_in_world), 1))], axis=1).T
    ).T
...
def draw_bev_projection(
    gui_image_handler, client_camera_pose, label_to_line_segments, front_camera_pose_interpolator
):
    """
    ...
    ego_pose_flu = np.concatenate(
        [ego_pose_opencv[:, 2:3], -ego_pose_opencv[:, 0:1], -ego_pose_opencv[:, 1:2], ego_pose_opencv[:, 3:]],
        axis=1,
    ...
    for label_name in label_to_visualize:
        ...
        x_coords, y_coords = world_coordinate_to_bev_image_coordinate(
            line_segments_in_world,
            world_to_ego,
            bev_map_h_pixel,
            bev_map_w_pixel,
            bev_map_h_meter,
        ...
        start_point_valid = (
            (x_coords[:, 0] >= 0)
            & (x_coords[:, 0] < bev_map_w_pixel)
            & (y_coords[:, 0] >= 0)
            & (y_coords[:, 0] < bev_map_h_pixel)
        )
        end_point_valid = (
            (x_coords[:, 1] >= 0)
            & (x_coords[:, 1] < bev_map_w_pixel)
            & (y_coords[:, 1] >= 0)
            & (y_coords[:, 1] < bev_map_h_pixel)
        )
        ...
        for x, y in zip(both_valid_x_coords, both_valid_y_coords, strict=True):
        ...
        for x, y in zip(either_valid_x_coords, either_valid_y_coords, strict=True):
            cv2.line(bev_map, (x[0], y[0]), (x[1], y[1]), color_uint8, 2)
    ...
    if t_min is not None and t_max is not None:
        ...
        valid_coord_idx = (
            (x_coords >= 0) & (x_coords < bev_map_w_pixel) & (y_coords >= 0) & (y_coords < bev_map_h_pixel)
        )
...
@click.command()
@click.option("--input_root", "-i", type=str, required=True, help="The root directory of the webdataset")
@click.option(
    "--novel_pose_folder",
    "-np",
    type=str,
...
def main(input_root, novel_pose_folder, dataset, clip_id):
    ...
    all_camera_to_front_camera = [
        np.linalg.inv(camera_poses[f"000000.pose.{all_cameras[0]}.npy"])
        @ camera_poses[f"000000.pose.{camera}.npy"]
    ...
    with server.gui.add_folder("Control"):
        ...
        gui_force_grounded_button = server.gui.add_checkbox(
            ...
            initial_value=True,
            hint="Force the recorded poses to be grounded" + "keep a fixed height as initial pose.",
        )
    ...
    gui_record_button.__setattr__(
        "front_camera_pose_interpolator", FreePoseAnimator(interp_type=InterpType.BEZIER)
    )
    ...
    def _(event: viser.GuiEvent):
        ...
        if gui_force_grounded_button.value:
            ...
            gui_record_button.frame_idx_to_front_camera_pose[gui_frame_slider.value] = grounding_pose(
                gui_record_button.frame_idx_to_front_camera_pose[gui_frame_slider.value],
    ...
    def _(event: viser.GuiEvent):
        ...
        for frame_idx in range(frame_num):
            ...
            for camera in all_cameras:
                ...
                interpolated_poses_sample[f"{frame_idx:06d}.pose.{camera}.npy"] = (
                    interpolated_poses_sample[f"{frame_idx:06d}.pose.camera_front_wide_120fov.npy"]
                    @ all_camera_to_front_camera[all_cameras.index(camera)]
                )
        ...
        client.add_notification(
            title="Success",
            body="Interpolated poses are saved!",
            loading=False,
            with_close_button=True,
            auto_close=3000,
            color="green",
    ...
    for label_name in settings["MINIMAP_TYPES"]:
        if MINIMAP_TO_TYPE[label_name] in ["polyline", "polygon"]:
            ...
            for vertices in minimap_data_wo_meta_info:
                line_segments.extend(list(zip(vertices[:-1], vertices[1:], strict=True)))
    ...
    for frame_idx in range(0, frame_num, dynamic_sampling_interval):
        ...
        for tracking_id in tracking_ids:
            ...
            outline = convert_dir_lwh_to_outline(
                heading_dir, left_dir, up_dir, center_xyz, object_lwh
            )  # shape [12, 3]
            line_segments_this_frame.extend(list(zip(outline[:-1], outline[1:], strict=True)))
    ...
    while True:
        ...
        label_to_line_segments["current_dynamic_objects"] = frame_idx_to_dynamic_object_line_segments[
            current_frame_idx_divisible_by_3
        ]
        ...
        camera_pose = camera_poses[
            f"{current_frame_idx:06d}.pose.{all_cameras[0]}.npy"
