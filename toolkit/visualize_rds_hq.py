...
def draw_bev_projection(gui_image_handler, client_camera_pose, label_to_line_segments):
    ...
    for label_name in label_to_visualize:
        ...
        line_segments_in_ego = (
            world_to_ego
            @ np.concatenate([line_segments_in_world, np.ones((len(line_segments_in_world), 1))], axis=1).T
        ).T  # shape [N*2, 4]
        ...
        valid_line_segments = (
            np.all(x_coords >= 0, axis=1)
            & np.all(x_coords < bev_map_w_pixel, axis=1)
            & np.all(y_coords >= 0, axis=1)
            & np.all(y_coords < bev_map_h_pixel, axis=1)
        )
...
@click.option("--dataset", "-d", type=str, default="rds_hq", help='The dataset name, "rds_hq" or "waymo"')
...
def main(input_root, novel_pose_folder, dataset, clip_id):
    ...
    def _(event: viser.GuiEvent):
        ...
        interpolator = FreePoseAnimator(interp_type=InterpType.BEZIER)
        ...
        for frame_idx in range(frame_num):
            ...
            if gui_force_grounded_button.value:
                interpolated_poses_sample[f"{frame_idx:06d}.pose.camera_front_wide_120fov.npy"] = (
                    grounding_pose(
                        interpolated_poses_sample[f"{frame_idx:06d}.pose.camera_front_wide_120fov.npy"],
                        reference_grounded_pose=camera_poses["000000.pose.camera_front_wide_120fov.npy"],
                        convention="opencv",
                    )
                )
    ...
    while True:
        ...
        for id, client in server.get_clients().items():
            draw_bev_projection(
                gui_image_handler, get_client_camera_pose_matrix(client), label_to_line_segments
            )
