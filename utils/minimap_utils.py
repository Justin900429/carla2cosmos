...
def create_minimap_projection(minimap_name, minimap_data_wo_meta_info, camera_poses, camera_model):
    """
    Args:
        minimap_name: str, name of the minimap
    ...
    cprint(
        f"Processing minimap {minimap_name} with shape {image_height}x{image_width}",
        "green",
    )
...
def cuboid3d_update_vertices_remove_others(cuboid3d):
    ...
    max_xyz = np.array(
        [
            cuboid3d["dimensions"]["x"] / 2.0,
            cuboid3d["dimensions"]["y"] / 2.0,
            cuboid3d["dimensions"]["z"] / 2.0,
        ]
    )
    min_xyz = -max_xyz
    ...
    yaw, pitch, roll = (
        cuboid3d["orientation"]["x"],
        cuboid3d["orientation"]["y"],
        cuboid3d["orientation"]["z"],
    )
    ...
    cuboid3d["vertices"] = [
        {"x": vertex[0], "y": vertex[1], "z": vertex[2]} for vertex in vertices_of_cuboid.tolist()
    ]
