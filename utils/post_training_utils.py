...
def overlay_condition_on_rgb(
    ...
    assert len(rgb_video) == len(condition_video), (
        f"RGB video and condition video must have the same length, but got {len(rgb_video)} and {len(condition_video)}"
    )
...
def overlay_condition_on_rgb_folder(
    ...
    assert len(rgb_videos) == len(condition_videos), (
        f"RGB videos and condition videos must have the same length, but got {len(rgb_videos)} and {len(condition_videos)}"
    )
    ...
    if USE_RAY:
        ...
        for rgb_video, condition_video in zip(rgb_videos, condition_videos):
            ...
            futures.append(
                overlay_condition_on_rgb.remote(
                    rgb_video.as_posix(),
                    condition_video.as_posix(),
